...
def read_chars_file(path: str | Path) -> str:
    return "".join(ch for ch in Path(path).read_text(encoding="utf-8") if not ch.isspace())
...
def font_display_name(font_path: str | Path) -> str:
    font = TTFont(str(font_path), lazy=True)
    names: list[str] = []
    for record in font["name"].names:
        if record.nameID in {1, 2, 4}:
            try:
                ...
                continue
    font.close()
    return next((name for name in names if name.strip()), Path(font_path).stem)
