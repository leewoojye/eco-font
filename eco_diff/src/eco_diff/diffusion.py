...
class DiffusionSchedule:
    ...
    def predict_x0_from_eps(self, xt: torch.Tensor, timesteps: torch.Tensor, eps: torch.Tensor) -> torch.Tensor:
        ...
        return (xt - sqrt_one_minus * eps) / sqrt_alpha.clamp_min(1e-8)

    @torch.no_grad()
    def p_sample(self, model, xt: torch.Tensor, condition: torch.Tensor, timestep: int) -> torch.Tensor:
        t = torch.full((xt.shape[0],), timestep, device=xt.device, dtype=torch.long)
        predicted_noise = model(xt, condition, t)
    ...
    def sample_loop(
        ...
        for timestep in schedule:
            x = self.p_sample(model, x, condition, int(timestep))
