...
class TimeResBlock(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, time_dim: int, dropout: float = 0.0) -> None:
        super().__init__()
        groups1 = _group_count(in_channels)
        groups2 = _group_count(out_channels)
...
class ConditionalDiffusionUNet(nn.Module):
    """Noise predictor for glyph-conditioned eco glyph diffusion.

    Input is the noisy eco glyph plus conditioning channels:
    original glyph, distance transform, skeleton, target saving, x coord, y coord.
    """
    ...
    def forward(self, noisy_eco: torch.Tensor, condition: torch.Tensor, timesteps: torch.Tensor) -> torch.Tensor:
        time_emb = self.time_mlp(sinusoidal_embedding(timesteps, self.time_dim))
        x = torch.cat([noisy_eco, condition], dim=1)
        skips = [self.input_block(x, time_emb)]
        current = skips[0]
...
def build_diffusion_model(config: dict) -> ConditionalDiffusionUNet:
    return ConditionalDiffusionUNet(
        condition_channels=int(config.get("condition_channels", 6)),
