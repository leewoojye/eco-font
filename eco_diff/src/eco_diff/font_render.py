...
def _load_font(font_path: str | Path, font_size: int) -> ImageFont.FreeTypeFont:
    try:
...
def render_glyph(
    ...
    """Render one glyph as a grayscale foreground image in [0, 1]."""
    if len(char) != 1:
        raise ValueError("render_glyph expects a single character")
    ...
    if width <= 0 or height <= 0:
        ...
        return RenderResult(char, arr, None, str(font_path), font_size, image_size)

    pad = padding * scale
    x = (canvas_size - width) // 2 - bbox[0]
    y = (canvas_size - height) // 2 - bbox[1]
    x = int(np.clip(x, pad - bbox[0], canvas_size - pad - width - bbox[0]))
    y = int(np.clip(y, pad - bbox[1], canvas_size - pad - height - bbox[1]))
    ...
    return RenderResult(char, arr, out_bbox, str(font_path), font_size, image_size)


def has_visible_glyph(image: np.ndarray, min_pixels: int = 12) -> bool:
...
def model_input_channels(glyph: np.ndarray, target_saving: float) -> np.ndarray:
    """Build LF-style local channels for the mask generator."""
    ...
    stacked = np.stack([glyph, dist, skel, target, xx, yy], axis=0)
